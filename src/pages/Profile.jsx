...
export default function Profile() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setGuiScale(profile.gui_scale || guiScale)
  ...
  async function handleSave() {
    setSaving(true)
    setFeedback('')
    const { error } = await updateProfile({ username: username.trim(), avatar_data_url: avatar || null, gui_scale: guiScale, dark_mode: darkMode })
    setSaving(false)
      ...
      <section className="profile-grid">
        <div className="glass-card profile-card">
          ...
          <div className="profile-actions">
            ...
            <button className="danger-button" onClick={() => setAvatar('')}><Trash2 size={17} /> Hapus Foto</button>
          </div>
        </div>

        <div className="glass-card settings-card">
          {feedback && <div className={`alert ${feedback.includes('berhasil') ? 'success' : 'error'}`}>{feedback}</div>}
          <label>
            Username
          ...
          <div className="setting-row">
            <div>
              <strong><Moon size={18} /> Dark Mode</strong>
          ...
          <div className="setting-row vertical-setting">
            ...
            <div className="scale-buttons">
              {[1, 2, 3].map((scale) => <button key={scale} className={guiScale === scale ? 'active' : ''} onClick={() => setGuiScale(scale)}>{scale}</button>)}
            </div>
          </div>

          <button className="primary-button full" onClick={handleSave} disabled={saving || username.trim().length < 3}><Save size={18} /> {saving ? 'Menyimpan...' : 'Simpan Profile'}</button>
        </div>
      </section>
    </PageTransition>
  )
